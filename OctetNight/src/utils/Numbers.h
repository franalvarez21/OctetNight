...
struct Numbers
{
  void print(uint8_t x, uint8_t y, int16_t number, uint8_t limiter = 2)
  {
    ...
    {
      printDigit(x, y, (number % multiplier) == 0 ? 0 : (number % multiplier) / (multiplier / 10), i, i == limiter - 1);
      multiplier *= 10;
    }
  }

  void printDigit(uint8_t x, uint8_t y, uint8_t number, uint8_t sector, bool bigger)
  {
    if (!(number == 0 && bigger))
    {
      switch (number)
